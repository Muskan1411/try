...
if(isset($_SESSION['uname'])) { 
	...
	function upload_image3($time) {
		...
		while($row=mysqli_fetch_array($res)) {
			...
			$sql4 = "Update approved_admins set ads_added=".($ads_added+1)." where CAID='$caid'";
			mysqli_query($conn, $sql4);
		}
	
		$sql2 = "Insert into advertisements (advertiser_name, description,coadmin_CAID,place_name,`300*600`,`160*600`,`728*90`,`720*300`,timeslot,timestamp,status) values ('$advertiser', '$description', '$caid', '$place', '$image', '$image1', '$image2', '$image3', $timeslot, '$currTime', 'no')";
		...
		if($status == 1) {
			echo "<script>alert('AD has been sent for approval.');
				window.location.href = 'maintainAdStatus.php';
			</script>";
		} else {
			echo "Ferf";
		}
